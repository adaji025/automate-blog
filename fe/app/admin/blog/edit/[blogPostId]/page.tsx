...
export default function AdminBlogEditPage() {
  ...
  useEffect(() => {
    async function fetchPost() {
      try {
        const response = await apiGet<BlogPost>(`/admin/blog/single/${blogPostId}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        response = await apiPutFormData(`/admin/blog/update/${blogPostId}`, formDataToSend);
      } else {
        response = await apiPut(`/admin/blog/update/${blogPostId}`, formData);
      }
          ...
          >
            ...
            <div className="mb-6">
              <label htmlFor="assets" className="mb-2 block font-medium text-black dark:text-zinc-50">
                New Images (optional, up to 3, max 2MB each)
              </label>
              <input
                type="file"
                id="assets"
